...
function Tables() {
      ...
      case "pending":
        return (
          <MDBox>
            ...
            <MDButton color="error" onClick={() => handleDelete(uid)}><DeleteIcon /></MDButton>
          </MDBox>
        );
      case "accepted":
        return (
  ...
  const rows = tableData.map((data, index) => ({
    ...
    status: (
      <MDBox>
        <MDButton color={checkColor(data.status)}>
          {data.status}
        </MDButton>
      </MDBox>
    ),
  ...
  const checkStatus = (status, rejection_reason) => {
    if (status === "accepted") {
      return (
        <MDBox sx={{ backgroundColor: '#b9f6ca', padding: '10px', borderRadius: '5px' }}>
          <MDTypography>
            Status: Your bonafide has been accepted
          </MDTypography>
        </MDBox>
      );
    } else if (status === "rejected") {
      return (
        <MDBox sx={{ backgroundColor: '#ffcdd2', padding: '10px', borderRadius: '5px' }}>
          <MDTypography>
            Status: Your bonafide has been rejected
          </MDTypography>
          <MDTypography>
      ...
      return (
        <MDBox sx={{ backgroundColor: '#FFD700', padding: '10px', borderRadius: '5px' }}>
          <MDTypography>
            ...
            <DialogContent>
              <DialogContentText>
                <Box sx={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                  ...
                  <MDTypography>
                    This is to certify that {data?.[0]?.fname} {data?.[0]?.lname} (Reg NO: {data?.[0]?.regno}, Aadhar No: {data?.[0]?.aadhar}) {check} MR.{data?.[0]?.fathername} is bonafide of our College, Studying in {year} year B.E/B.Tech {data?.[0]?.dept} during academic year {academicYear}
                  </MDTypography>
                    ...
                    <FormControl fullWidth sx={{ minWidth: '200px' }}>
                      ...
                      >
                        {[...Array(10)].map((_, index) => (
                          <MenuItem key={index} value={`2024-${2025 + index}`}>{`2024-${2025 + index}`}</MenuItem>
                        ))}
                      </Select>
                    </FormControl>
                    <FormControl fullWidth sx={{ minWidth: '200px' }}>
                      ...
                      >
                        {[
                          ...
                          <MenuItem key={index} value={purpose}>{purpose}</MenuItem>
                        ))}
                      </Select>
                    </FormControl>
                  </Box>
            ...
            <DialogContent>
              <Grid container spacing={2}>
                <Grid item xs={12} sm={6}>
                  ...
                  <MDBox>
                    {checkStatus(rowData?.[0]?.status, rowData?.[0]?.rejection_reason)}
                  </MDBox>
                </Grid>
